...
def draw_bbox(frame, boxes, class_names, colors):
    for box in boxes:
        ...
        small_width = rect_coords[0] - x1
        small_height = rect_coords[1] - y2

        small_rect_coords = (x1 + small_width+10, y2 + small_height // 13)

        # Drawing the background rectangle and the label text
        cv2.rectangle(frame, (x1, y2), small_rect_coords, color, -1, cv2.LINE_AA)
        cv2.putText(frame, label, (x1 + 10, y2-10 ), 0, 1, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
